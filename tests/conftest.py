...
class FakeStreamlitModule(types.ModuleType):
    def __init__(self):
        ...
        def _columns(spec, *a, **k):
            ...
            return [_DummyContextManager() for _ in range(n)]
        ...
        self.container = lambda *a, **k: _DummyContextManager()
        ...
        # Auth stubs — always treat as logged in during tests
        self.user = types.SimpleNamespace(is_logged_in=True, name="Test User", email="test@example.com")
...
def make_fake_st(text_inputs=None, mode=None):
    ...
    def st_cache_data(ttl=None, show_spinner=True, **kwargs):
        def deco(func):
            return func
